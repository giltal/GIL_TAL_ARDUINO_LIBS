...
boolean Adafruit_FT6206::begin(uint8_t threshhold)
{
	...
	writeRegister8(0xA4, 0);
	writeRegister8(0x00, 0);
	writeRegister8(0x86,0x1);
	writeRegister8(0xA5, 0x0);
	
	chipID = readRegister8(FT6206_REG_VENDID);
#if 1  
	Serial.print("Vend ID: "); Serial.println(chipID);
	...
	Serial.print("Thresh 0x81: "); Serial.println(readRegister8(0x81));
#endif

	if ((readRegister8(FT6206_REG_VENDID) == 17) && ((readRegister8(FT6206_REG_CHIPID) == 54) || (readRegister8(FT6206_REG_CHIPID) == 100)))
		return true; // Gil - Fix for FT6236, FT6336G
	else
		return false;
}
...
int  Adafruit_FT6206::getTouchedPoints(uint16_t *x1, uint16_t *y1, uint16_t *x2, uint16_t *y2)
{
	uint8_t i2cdat[10];
	...
	if (i2cdat[0] == 2)
	{
		...
		Wire.endTransmission();
		Wire.beginTransmission(FT6206_ADDR);
		Wire.requestFrom((byte)FT6206_ADDR, 10);
		Wire.readBytes(i2cdat, 10);
		Wire.endTransmission();
	...
	if (i2cdat[0] == 1)
	{
		...
		Wire.endTransmission();
		Wire.beginTransmission(FT6206_ADDR);
		Wire.requestFrom((byte)FT6206_ADDR, 4);
		Wire.readBytes(i2cdat, 4);
		Wire.endTransmission();
